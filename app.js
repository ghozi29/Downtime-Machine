...
function computeAllKPIs() {
  ...
  filteredRecords.forEach(r => {
    ...
    } else if (r.downtimeHours) totalDowntime += r.downtimeHours;
  });
...
function renderBarChart() {
  ...
  if (!ctx) return;
  const data = {};
  allMaintenanceRecords.forEach(r => { const m=r.machineName||"Unknown"; data[m]=(data[m]||0)+(r.downtimeHours||0); });
...
function renderLineChart() {
  ...
  if (!ctx) return;
  const data = {};
  allMaintenanceRecords.forEach(r => { if(r.failureStart){ const d=new Date(r.failureStart); const k=`${d.getFullYear()}-${String(d.getMonth()+1).padStart(2,'0')}`; data[k]=(data[k]||0)+(r.downtimeHours||0); }});
