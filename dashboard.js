...
async function loadDashboard(area) {
  ...
  try {
    ...
    const mttr = records.length > 0
      ? records.reduce((sum,r) => sum + (r.repairTime||0),0)/records.length : 0;
    document.getElementById("mttr").innerText = mttr.toFixed(2);

    const sorted = records.map(r=>({op:new Date(r.operationStart),fail:new Date(r.failureStart)})).sort((a,b)=>a.op-b.op);
    let mtbf = 0;
    if(sorted.length>1){
      let sum=0;
      for(let i=1;i<sorted.length;i++) sum+=(sorted[i].op-sorted[i-1].fail)/3600000;
