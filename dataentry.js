...
function setupMaintenanceForm() {
  ...
  form.addEventListener("submit", async (e) => {
    ...
    try {
      ...
      if (!machineName || !category || technicians.length === 0 || !operationStart || !failureStart || !maintenanceStart || !maintenanceEnd) {
        alert("Data belum lengkap!");
        return;
      }

      // ===== TIME CONVERSION =====
      const opStartISO = new Date(operationStart).toISOString();
      const failStartISO = new Date(failureStart).toISOString();
      const maintStartISO = new Date(maintenanceStart).toISOString();
      const maintEndISO = new Date(maintenanceEnd).toISOString();

      const repairTime = (new Date(maintEndISO) - new Date(maintStartISO)) / 3600000;
      ...
      await push(ref(db, `area/${area}/records`), {
        ...
        repairTime,
        note,
        operationStart: opStartISO,
        failureStart: failStartISO,
        maintenanceStart: maintStartISO,
        maintenanceEnd: maintEndISO,
      ...
      alert("✅ Data mesin tersimpan & workload terupdate");
      form.reset();
